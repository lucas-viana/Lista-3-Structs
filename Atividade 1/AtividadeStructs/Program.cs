...
class Program
{
    ...
    static void buscarGenero(List<TipoBanda> lista, string nomeBusca)
    {
        ...
        for (int i = 0; i < qtd; i++)
        {
            if (lista[i].genero.ToUpper().Contains(nomeBusca.ToUpper()))
            {
                Console.WriteLine("\t*** Dados da banda ***");
                Console.WriteLine("Nome:" + lista[i].nome);
                Console.WriteLine("Genero:" + lista[i].genero);
    ...
    static void alterarBanda(List<TipoBanda> lista, string nomeBusca)
    {
     ...
     //----------------------
    static void removerBanda(List<TipoBanda> listaBanda, string nomeBusca)
    {
        int qtd = listaBanda.Count();
        bool flag = false;

        for (int i = 0; i < qtd; i++)
        {
            if (listaBanda[i].nome.ToUpper().Equals(nomeBusca.ToUpper()))
            {
                Console.WriteLine($"Tem certeza de que deseja remover a banda {listaBanda[i].nome}, digite 's' ou 'S' se sim.");
                char validation = char.Parse(Console.ReadLine());
                if (validation == 'S' || validation == 's')
                {
                    listaBanda.RemoveAt(i);
                }
                else Console.WriteLine("Operacao cancelada");
                flag = true;
                break;
            }
    ...
    static int somarIntegrantes(List<TipoBanda> listaBanda) { 
        int qtd = listaBanda.Count();
        int somaIntegrantes = 0;
        for (int i = 0; i < qtd; i++)
        {
            somaIntegrantes += listaBanda[i].integrantes;
        }
        return somaIntegrantes;
    }
    ...
    static int menu()
    {
        ...
        Console.WriteLine("8-Somar integrantes");
        ...
        do
        {
            ...
            switch (op)
            {
                ...
                case 4:
                    ...
                    break;
                case 5:
                    Console.Write("Nome do genero para busca:");
                    string buscaGenero = Console.ReadLine();
                    buscarGenero(listadeBandas, buscaGenero);
                    break;
                ...
                case 7:
                    ...
                    break;
                case 8:
                    Console.Write("Soma dos integrantes: ");
                    Console.WriteLine(somarIntegrantes(listadeBandas));
                    break;
                case 0:
